<template>
    <div>
        <div :style="name==='list2'?'display: block;':'display: none;'">
                    ...
                    <div class="col-xs-4 col-sm-4 col-md-4 col-lg-4" v-for="(item,index) in TopList" :key="index">
                        <div class="media">
                            <div class="media-left media-middle">
                                <router-link :to="{path:'/article/list2/detail', query:{id:item.ID}}" v-if="item.LinkUri===''">
                            ...
                            <div class="media-body pl-15">
                                <div class="lh20" style="height:65px;">
                                    <router-link class="c333" :to="{path:'/article/list2/detail', query:{id:item.ID}}" v-if="item.LinkUri===''">
                                        {{item.DocTitle}}
                                    </router-link>
	...
	export default {
		...
		methods: {
			...
			getData (p) {
				...
				that.loading = true
				setTimeout(function () {
					request.getRequest("GetNewsCenterList", {pageIndex: p})
					.then(res => {
					...
					if (p === 1) {
						request.getRequest("GetNewsCenterList", {isTop: 1})
						.then(res => {
